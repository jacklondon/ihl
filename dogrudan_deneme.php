            ...
            <div class="container">
               ...
               <div class="col-sm-3">
                     ...
                     <div class="form-group">
                        ...
                        <div class="multiselect" style="margin-top: 10%;">
                           <div class="selectBox" onclick="showVites()">
                              <select style="height:1.8em;">
                              <option>Vites Tipine Göre</option>
                              </select>
                              <div class="overSelect"></div>
                           </div>
                           <div id="vites"> 
                              <label for="Düz Vites">                          
                              <input type="checkbox" name="vites[]" value="Düz Vites" />Düz Vites</label><br>
                              <label for="Çekme Belgeli"> 
                              <input type="checkbox" name="vites[]" value="Otomatik Vites" />Otomatik Vites</label><br>
                           </div>
                        </div>
                        <div class="multiselect" style="margin-top: 10%;">
                           <div class="selectBox" onclick="showYakit()">
                              <select style="height:1.8em;">
                              <option>Yakıt Tipine Göre</option>
                              </select>
                              <div class="overSelect"></div>
                           </div>
                           <div id="yakit"> 
                              <label for="Benzinli">                          
                              <input type="checkbox" name="yakit[]" value="Benzinli" />Benzinli</label><br>
                              <label for="Dizel"> 
                              <input type="checkbox" name="yakit[]" value="Dizel" />Dizel</label><br>
                              <label for="Benzin+Lpg"> 
                              <input type="checkbox" name="yakit[]" value="Benzin+Lpg" />Benzin+Lpg</label><br>
                              <label for="Hybrit"> 
                              <input type="checkbox" name="yakit[]" value="Hybrit" />Hybrit</label><br>
                              <label for="Elektrikli"> 
                              <input type="checkbox" name="yakit[]" value="Elektrikli" />Elektrikli</label><br>
                           </div>
                        </div>
                        <div class="multiselect" style="margin-top: 10%;">
                           <div class="selectBox" onclick="showHasar()">
                              <select style="height:1.8em;">
                           ...
                           <div id="hasar"> 
                              <label for="Çarpma, Çarpışma">                          
                              <input type="checkbox" name="hasar[]" value="1" />Çarpma, Çarpışma</label><br>
                              <label for="Dizel"> 
                              <input type="checkbox" name="hasar[]" value="2" />Teknik Arıza</label><br>
                              <label for="Sel/Su Hasarı"> 
                              <input type="checkbox" name="hasar[]" value="3" />Sel/Su Hasarı</label><br>
                              <label for="Yanma Hasarı"> 
                              <input type="checkbox" name="hasar[]" value="4" />Yanma Hasarı</label><br>
                              <label for="Çalınma"> 
                              <input type="checkbox" name="hasar[]" value="5" />Çalınma</label><br>
                              <label for="Diğer"> 
                              <input type="checkbox" name="hasar[]" value="6" />Diğer</label><br>
                              <label for="Hasarsız"> 
                              <input type="checkbox" name="hasar[]" value="7" />Hasarsız</label><br>
                           </div>
                        </div>
                        ...
                        <div class="form-check" style="margin-top: 5%;">
                           <h5>Kilometreye Göre</h5>
                        </div>
                        <div class="form-group">
                           <label for="exampleInputEmail1">En düşük</label>
                           <input type="text" name="kucuk_km" class="form-control">
                        </div>
                        <div class="form-group">
                           <label for="exampleInputPassword1">En yüksek</label>
                           <input type="text" name="buyuk_km" class="form-control" >
                        </div>
                  ...
                  if(isset($_POST['filtrele'])){                    
                    ...
                    if($f_tarih=="1"){
                     ...
                     $where .= "AND  kilometre BETWEEN $f_kucuk_km AND $f_buyuk_km";
                     }
                           ...
                           <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_alt_alan">
                              ...
                              <div class="col-xs-12 col-sm-12 col-md-5 col-lg-5 ilan_karti_teklif_dis">
                                 ...
                                 <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_teklif_btnlar">
                                    ...
                                    <a onclick="dogrudan_arttir(<?=$filtre_oku['id']?>)" style="text-decoration: none; color:#ffffff;" href="arac_detay.php?id=<?= $filtre_oku['id'] ?>&q=dogrudan">İNCELE</a>
                                    </div>
                                 </div>
                              </div>
                           </div>
               ...
               $reklam_cek = mysql_query("SELECT * FROM reklamlar WHERE baslangic_tarihi <= '".$dateTime."' AND bitis_tarihi >= '".$dateTime."'");     
               $reklam_say = mysql_num_rows($reklam_cek);
               $yayin_sayi = $sql_sayi / $reklam_say;
               $yayinla_sayi = floor($yayin_sayi);
               ...
               ?>
                  ...
                  <?php  
                  if($yayinla_sayi%$number == 0){

                     while($reklam_oku = mysql_fetch_array($reklam_cek)){
                        ?>
                        <div class="container">
                           <a href="<?= $reklam_oku['url'] ?>">
                           <img src="reklamlar/<?= $reklam_oku['resim'] ?>" alt="">
                           </a>
                        </div>
                    }
                        <?php 
...
Insert advertisement between entries from MySQL using PHP
while($reklam_oku = mysql_fetch_array($reklam_cek)){
                     $reklamID = $reklam_oku['id'];
                     $reklam_array = array();
                     array_push($reklamID,$reklam_array);
                     $array_count = count($reklam_array);
                     }
                     $reklam_cek_2 = mysql_query("SELECT * FROM reklamlar WHERE baslangic_tarihi <= '".$dateTime."' AND bitis_tarihi >= '".$dateTime."' ORDER BY RAND()");
                     $advertisment_block .= '<div class="container">
                        <a href="'.$reklam_oku["url"].'">
                        <img src="reklamlar/'.$reklam_oku["resim"].' " alt="" style="width:100%; height:auto;">
                        </a>
                     </div>'; }
                     ...
                     $reklam_array = array();
                     while($reklam_oku = mysql_fetch_array($reklam_cek)){
                        $reklam_resim = $reklam_oku['resim'];
                       
                        array_push($reklam_array,$reklam_resim);
                        $array_count = count($reklam_array);
                      
                        $advertisment_block .= '<div class="container">
                           <a href="'.$reklam_oku["url"].'">
                           <img src="reklamlar/'.$reklam_array[0].' " alt="" style="width:100%; height:auto;">
                           </a>
                        </div>'; }
