<?php 
   ...
   include('../ayar.php');
    ...
    include 'alert.php';
    $yorumlari_cek = mysql_query("select * from yorumlar");
  ...
  <ul class="navbar-nav mr-auto" >
      ...
      <?php  include 'template/header.php'; ?>
         ...
         <div class="row">
            ...
            <div class="col-sm-8">
               <?php while($yorumlari_oku = mysql_fetch_assoc($yorumlari_cek)){ 
                  $yorumlu_ilan_cek = mysql_query("select * from ilanlar where id = '".$yorumlari_oku['ilan_id']."' limit 1");
                  ...
                  while($yorumlu_ilan = mysql_fetch_array($yorumlu_ilan_cek)){
                     $marka_cek = mysql_query("select * from marka where markaID = '".$yorumlu_ilan['marka']."'");
                     $marka_oku =mysql_fetch_assoc($marka_cek);
                     $marka_adi = $marka_oku['marka_adi'];
               ?>
               <div class="card mt-2" style="border: 1px solid #000000;">
                  <h5 class="card-header" style="background-color: rgb(255, 192, 0); color:#000000; font-weight:750;"> <?= $yorumlu_ilan['model_yili']." / ".$marka_adi ." / ".$yorumlu_ilan['tip'] ?> / MERCEDES C180 AMG</h5>
                  <div class="card-body" style="border-bottom: 1px solid #000000;">
                     <p class="card-text" style="color: #000000;" ><b>Yorum Sahibi : <?= $uye_adi ?> </b> <b style="margin-left:50px;">Tarih : 20.04.2021</b></p> 
                  </div>
                  <div class="card-body" style="border-bottom: 1px solid #000000; background-color:#e8e8e8;">
                     <p class="card-text"><?= $yorumlari_oku['yorum'] ?></p>
                  </div>
                  <div class="card-body" style="border-bottom: 1px solid #000000;">
                     <p class="card-text" style="color: #000000;"><b>Cevap : Pert Dünyası Ekibi </b> <b style="margin-left:50px;">Tarih : <?= $newDate ?></b></p> 
                  </div>
                  <div class="card-body" style="background-color: #e8e8e8;">
                     <p class="card-text"><?= $yorumlari_oku['cevap'] ?></p>
                  </div>
               </div>
               <?php } }?>
            </div>
