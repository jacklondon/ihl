    ...
    <ul class="navbar-nav mr-auto" >
      ...
      <?php while($kullanici_oku = mysql_fetch_array($kullanici_cek)){ 
        ...
        if($uye_demo_tarihi == $bugun){
          ...
          mysql_query("UPDATE teklif_limiti SET teklif_limiti = '".$ust_limiti."', standart_limit = '".$ust_standart."',
          luks_limit = '".$ust_luks."' WHERE uye_id = '".$kullanici_oku['id']."'");
        }
  ...
  margin:0px!important;
    ...
    <div class="container" style="margin-top:2%; margin-bottom:5%;">
      <div class="row">
        ...
        <?php 
        // $odeme_bekleyen_cek = mysql_query("SELECT * FROM kazanilan_ilanlar WHERE durum = 0 AND uye_id = '".$kullanici_oku['id']."'");	
        $odeme_bekleyen_cek = mysql_query("SELECT * FROM kazanilan_ilanlar LEFT JOIN ilanlar ON kazanilan_ilanlar.ilan_id = ilanlar.id 
        WHERE kazanilan_ilanlar.uye_id = '".$kullanici_oku['id']."' AND ilanlar.id = kazanilan_ilanlar.ilan_id AND kazanilan_ilanlar.durum = 0");
                        ...
                        <table class="table table-bordered">
                          <tbody>
                            ...
                            <tr style="background-color: transparent;">
                              ...
                              <td><span class="statu_baslik">Statü:</span> Onay Bekliyor</td>
                              <td><span class="statu_baslik">Açıklamalar:</span> <?= $odeme_bekleyen_oku['aciklama'] ?></td>
                            </tr>
                            <tr style="background-color: transparent;">
                              <td colspan="3"><span class="statu_baslik">Teklifim:</span> <?= money($odeme_bekleyen_oku['kazanilan_teklif']) ?> ₺ </td>
                            </tr>
                            <tr style="background-color: transparent;">
                              <td colspan="3">
                                <a style="float: right;" href="../arac_detay.php?id=<?= $oku_bakam['id'] ?>&q=ihale"><button type="button" class="btn btn-sm" style="background-color: #424242; color:#ffffff;">İNCELE</button></a>
                              </td>
                            </tr>
                          </tbody>
                      </table>
