<?php 
    ...
    $getUserInfo = mysql_query("SELECT * FROM user WHERE user_token = '$uye_token' AND user_token <> '0'");
    $userInfo = mysql_fetch_object($getUserInfo);
   
    $getAllFavoriteRecords = mysql_query("SELECT f.*  FROM favoriler AS f INNER JOIN ilanlar AS i ON f.ilan_id = i.id WHERE f.uye_id = '$userInfo->id'");
					...
					while($favori_oku = mysql_fetch_array($favorileri_cek)){
						...
						while($row_teklif=mysql_fetch_object($sql_teklif)){
							...
							if($row_teklif->teklif != "" ){
								$en_yksk ='<uk style="color: red; text-align:center;">'.money($row_teklif->teklif).' ₺ teklif verdiniz.</uk>';
							}
							*/
							if($row_teklif->teklif != "" ){
						...
						while($row_teklif=mysql_fetch_object($sql_teklif)){
							if($kapali_durum=="2"){
								if($row_teklif->teklif != "" ){
									$en_yksk_kapali ='<uk style="color: red;float: right; text-align:right;" id="benim_teklif_'.$ihale_oku['id'].'">'.money($row_teklif->teklif).' ₺ teklif verdiniz.</uk>';
								}
								/*if($row_teklif->teklif == ""){
									$en_yksk_kapali ='<uk style="color: red; text-align:center;">Henüz teklif vermediniz.</uk>';
						...
						if($ihale_oku['sigorta'] != $yasaki_sigorta_id ){
							if($secilen_yetki_id==2 ){
								if($detay_gorur==1){ 
									...
									<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_dis"  style="<?php if($ihale_oku['ihale_tarihi'].' '. $ihale_oku['ihale_saati'] < date("Y-m-d H:i:s") ){ echo('background-color:gray; opacity: 0.5;'); } ?>">
												...
												<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_alt_alan">
													...
													<div class="col-xs-12 col-sm-12 col-md-5 col-lg-5 ilan_karti_teklif_dis">
														...
														<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_teklif_fiyat">															
															<div id="ilan_fiyat_bilgisi_<?= $ihale_oku["id"] ?>">
															<?php 
																if($ihale_oku["ihale_tarihi"]." ".$ihale_oku["ihale_saati"] > date('Y-m-d H:i:s')){ ?>
																	<?php 
																	if($ihale_oku['ihale_turu']==1){ 
																		if($ilan_status == 1 && $user_package_status == 1){
																			echo money($ihale_oku['son_teklif']). " ₺";
																		}else{
																			echo '<i style="color:#000" class="fas fa-lock"></i>';
																		}
																	?>
																	<?php }else{ 
																		if($ilan_status == 1 && $user_package_status == 1){
																			echo money($ihale_oku['acilis_fiyati']). " ₺";
																		}else{
																			echo '<i style="color:#000" class="fas fa-lock"></i>';
																		}
																	} ?>
									...
									<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_dis" style="background-color:gray !important; opacity: 0.5;">
										<div style="color:#000;background-color:gray !important; opacity: 0.5;" class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_baslik">
											<?php 
												if(strlen($ihale_oku['model_yili']." ".$setMarka['marka_adi']." " . $ihale_oku['tip']) > 120){
													$ilan_karti_baslik_bas = substr($ihale_oku['model_yili']." ".$setMarka['marka_adi']." " . $ihale_oku['tip'],0,120)."...";
												}else{
													$ilan_karti_baslik_bas = $ihale_oku['model_yili']." ".$setMarka['marka_adi']." " . $ihale_oku['tip'];
												} 
											?>
							   ...
							   <?php } 
									...
									<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_dis"  style="<?php if($ihale_oku['ihale_tarihi'].' '. $ihale_oku['ihale_saati'] < date("Y-m-d H:i:s") ){ echo('background-color:gray; opacity: 0.5;'); } ?>">
										<?php  if($ihale_oku['ihale_turu']==1){$background_color = "#00a2e8";}else{$background_color="orange";}  ?>
										<?php 
												...
												<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_alt_alan">
													<div class="col-xs-12 col-sm-12 col-md-7 col-lg-7 ilan_karti_notlar_dis">
														<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_not_alan">
															...
															?>
														<b><?= $ihale_oku['uyari_notu']."<br>".$onay2 ?></b>
														</div>
														...
														<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_teklif_btnlar">
															<div class="ilan_karti_teklif_btn" style="background-color:<?= $background_color ?>">
																<?php if($ihale_oku['durum']=="-1"){ ?>
																	<span style="text-decoration: none; color:#ffffff;" >TEKLİF VER</span>                                                           
																<?php } else { ?>
					...
					var x = setInterval(function() 
					{
						jQuery.ajax({
							...
							success: function(response) {
								...
								if(response.sure_bitmis == 1){
									document.getElementById("ilan_fiyat_bilgisi_"+id).innerHTML = '<i style="color:#000" class="fas fa-lock"></i>';
								}else{
			...
			function denem(ilan_id){
				...
				if(verilen_teklif ==""  ){
					...
					$.ajax({
						...
						success: function(data) {			
							// $('#benim_teklif_'+ilan_id).html(verilen_teklif+" ₺ teklif verdiniz");			
							if(data.status==200){   			
								alert(data.message);
								$('#benim_teklif_'+ilan_id).html(formatMoney(verilen_teklif)+" ₺ teklif verdiniz");
								$('#teklifVer'+ilan_id).modal('hide');
