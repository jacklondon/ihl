   ...
   <body>
     ...
     if($site_acilis_popup_icin_say == 0){
          ...
          if($site_acilis_popup_icin_oku['tarih'] < $siteye_giris_tarih_before){
			...
			$sistem_devam_bilgileri_cek = mysql_query("SELECT * FROM `user` WHERE user_token = '$uye_token'");
				...
				<div class="row">
					...
					<div class="col-sm-8">
						<strong class="mesaj"></strong>
								...
								<div class="row">
									<div class="col">
										<div class="form-group">
											<label for="IDofInput">Ad Soyad</label>
										...
										<div class="form-group" >
											...
											<input type="text" onkeypress="return isNumberKey(event)" class="form-control" maxLength="11" id="tc_kimlik" name="tc_kimlik" value="<?= $kullanici_oku['tc_kimlik'] ?>"<?php  if($kullanici_oku['tc_kimlik'] !=''){ echo 'readonly';}  ?> >
									...
									<div class="col">
										...
										<div class="form-group">
											...
											<text onclick="check('ilg4');" style="margin-right:2%;">Otomobil</text><input type="checkbox" id="ilg4" name="ilgilendigi[]" value="Otomobil" <?php if($drm4==true) echo 'checked'; ?> /><br>
											<text onclick="check('ilg5');" style="margin-right:2%;">Çekici ve Kamyon</text><input type="checkbox" id="ilg5" name="ilgilendigi[]" value="Çekici ve Kamyon" <?php if($drm5==true) echo 'checked'; ?> /><br>
											<text onclick="check('ilg6');" style="margin-right:2%;">Dorse</text><input type="checkbox" id="ilg6" name="ilgilendigi[]" value="Dorse" <?php if($drm6==true) echo 'checked'; ?> /><br>
											<text onclick="check('ilg7');" style="margin-right:2%;">Motosiklet</text><input type="checkbox" id="ilg7" name="ilgilendigi[]" value="Motosiklet" <?php if($drm7==true) echo 'checked'; ?> /><br>
											<text onclick="check('ilg8');" style="margin-right:2%;">Traktör</text><input type="checkbox" id="ilg8" name="ilgilendigi[]" value="Traktör" <?php if($drm8==true) echo 'checked'; ?> /><br>
											<text onclick="check('ilg9');" style="margin-right:2%;">Sadece lüks segment</text><input type="checkbox" id="ilg9" name="ilgilendigi[]" value="Sadece lüks segment" <?php if($drm9==true) echo 'checked'; ?> /><br>
											
										</div>
									  <!-- <div class="form-group">
											<label for="IDofInput">Vergi Dairesi Adı</label>
											<input type="text" class="form-control" required id="vergi_dairesi" name="vergi_dairesi" value="<?= $kullanici_oku['vergi_dairesi'] ?>">
										</div>                     
										<div class="form-group">
											<label for="IDofInput">Vergi Numarası</label>
											<input type="text" class="form-control" required id="vergi_dairesi_no" name="vergi_dairesi_no"  value="<?= $kullanici_oku['vergi_dairesi_no'] ?>">
							...
							<button type="button" onclick="guncelle2();" class="btn btn-primary" >KAYDET VE BAŞVURUYU TAMAMLA</button>
						</form>
					</div>
				</div>
			</div>
		...
		<script>
			...
			function guncelle2(){
				...
				if(tc_kimlik == ""){
					...
					jQuery.ajax({
					url: "https://ihale.pertdunyasi.com/uye_panel/islemler/profil_duzenle.php",
					type: 'POST',
					data: form_data,
					dataType: "JSON",
					success: function(response) {
						...
						if (response.status != 200) {
							...
							window.location.href = 'https://ihale.pertdunyasi.com/uye_panel/islemler/gold_uyelik_basvuru.php?id=21';
							//location.reload();
						}
					},
					cache: false,
			...
			/* $('#kullaniciyi').on('click', function () {
				jQuery.ajax({
					...
					data: {
						...
						ilgilendigi:document.getElementById('ilgilendigi').value,
						//vergi_dairesi_no:document.getElementById('vergi_dairesi_no').value,
						//vergi_dairesi:document.getElementById('vergi_dairesi').value,
						adres:document.getElementById('adres').value,
						kargo_adresi:document.getElementById('kargo_adresi').value,
					...
					success: function(response) {
						...
						if (response.status != 200) {
							...
							alert("Başarıyla guncellendi");
							location.reload();
							 //window.location="uye_panel/profili_duzenle.php";
