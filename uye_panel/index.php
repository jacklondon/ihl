...
<body>
  ...
  $kazanilan_cek = mysql_query("
		...
		from
			kazanilan_ilanlar
		inner join ilanlar as i
			on i.id=kazanilan_ilanlar.ilan_id 
		inner join marka as m
			on m.markaID=i.marka
		inner join (select * from ilan_resimler where durum=1) as r
			on r.ilan_id=i.id
		where 
			kazanilan_ilanlar.uye_id='" . $kullanici_oku['id'] . "' and 
			kazanilan_ilanlar.modal_durum=0 and 
			kazanilan_ilanlar.durum=3
		order by
			kazanilan_ilanlar.id desc	
		limit 1
	");
  */
            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_dis">
              ...
              <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ilan_karti_baslik ">
                ...
                if ($odeme_bekleyen_oku['son_odeme_tarihi'] != "0000-00-00") { ?>
                  <span class="<?php if ($odeme_bekleyen_oku['son_odeme_tarihi'] < date('Y-m-d')) {
                                  echo "blink";
                                } ?>"> SON ÖDEME TARİHİ : <?= date("d-m-Y", strtotime($odeme_bekleyen_oku['son_odeme_tarihi'])) ?> </span>
                <?php } else { ?>
                    ...
                    <tbody>
                      ...
                      <tr style="background-color: transparent;">
                        <td colspan="3">
                          ...
                          <a href="../arac_detay.php?id=<?= $odeme_bekleyen_oku['ilan_id'] ?>&q=ihale"><button type="button" class="btn btn-sm" style="background-color: #424242; color:#ffffff; float:right;">İNCELE</button></a>
                        </td>
                      </tr>
                    </tbody>
                  </table>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="form-group">
            <label for="exampleFormControlTextarea1">Pert Dünyası ile yaptığınız alışverişle ilgili yorumunuz</label>
            <textarea class="form-control" id="kazanilan_yorum" rows="3" onclick="textarea_bosalt()">İyi bir alışverişti. Güvenle alışveriş yapabilirsiniz.</textarea>
          </div>
        </div>
  ...
  <script>
    function textarea_bosalt() {
      var $yorum = $('#kazanilan_yorum').val();
      if ($yorum == "İyi bir alışverişti. Güvenle alışveriş yapabilirsiniz.") {
        $('#kazanilan_yorum').val("");
      }
      ...
      $.ajax({
        ...
        success: function(response) {
          jQuery.each(response.data, function(index, value) {
            // if (value.yorum_yapsin == 1 && value.id != null) {
