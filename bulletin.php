<?php 
   ...
   $duyuru_cek = mysql_query("SELECT * FROM duyurular WHERE durum = 1 order by eklenme_zamani desc");
?>
<!doctype html>
<html lang="en">
...
<body data-spy="scroll" data-target=".site-navbar-target" data-offset="300">
  <div class="site-wrap" id="home-section">
    ...
    <div class="site-section bg-light">
      <div class="container">
	  <h1 style="text-align:center; padding-top:25px; padding-bottom: 10px;" >Blog</h1>	
        <div class="row">
		
        <?php while($duyuru_oku = mysql_fetch_array($duyuru_cek)){ 
          $ilk_icerik = $duyuru_oku['kisa_icerik'];
          $icerik = substr($ilk_icerik , 0, 100);    
        ?>
          <div class="col-lg-4 col-md-6 mb-4">
            <div class="post-entry-1 h-100 shadow">
              <a href="duyuru_detay.php?id=<?= $duyuru_oku['id'] ?>">
                <img src="duyurular/<?= $duyuru_oku['resim'] ?>" alt="Image" class="img-fluid">
              </a>
              <div class="post-entry-1-contents">
                <a href="duyuru_detay.php?id=<?= $duyuru_oku['id'] ?>"><?= $duyuru_oku['baslik'] ?>
                <span class="meta d-inline-block mb-3"><?= date("d-m-Y H:i:s", strtotime($duyuru_oku['eklenme_zamani'])) ?> <span class="mx-2"><i class="icon-user"></i></span> <a
                    href="#">Admin</a>                  
                </span>
                <p><?= $icerik . "</p><a href='duyuru_detay.php?id=".$duyuru_oku['id']."'>...Devamını oku</a>" ?>
              </div>
            </div>
          </div>
          <?php } ?>
