<?php 
	...
	for($i=0;$i<count($chat_rooms);$i++){
		...
		// 	}
		// }
		$verileri_bas .= '<div class="card-header '.$class.'" style="opacity: '.$opacity.';" id="message_room_'.$chat_rooms[$i]["id"].'"  data-toggle="modal" data-target="#mesaj_sayfa_modal"  onclick="getChatRoomMessages(\''.$chat_rooms[$i]["id"].'\',\''.$benim_chat_id.'\')">
     ...
     if($site_acilis_popup_icin_say == 0){
          ...
          if($site_acilis_popup_icin_oku['tarih'] < $siteye_giris_tarih_before){
			...
			<script>
						...
						success: function(response) {
							jQuery.each(response.data, function(index, value){
								...
								if(value.arac_detay[0].devam_ediyor == 0){
									var $opacity = "0.5";
									var $arac_detay = `<a onclick="alert('İlan yayından kaldırılmış')" style="float:right">${value.arac_detay[0].detay}</a>`;
								}
				...
				setInterval(function() {
				   chat_odalari_getir("<?= $benim_chat_id ?>");
				}, 5000);
