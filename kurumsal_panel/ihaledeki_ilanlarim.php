...
$ilan_cek = mysql_query("SELECT * FROM ilanlar WHERE ihale_sahibi = '" . $uye_token . "' order by id desc");
               ...
               <table class="table table-bordered">
                  <thead>
                     <tr style="background-color: #ffffff;">
                        ...
                        <th scope="col" class="yazi">Araç</br>Detaları</th>
                        <th scope="col" class="yazi">Şehir</th>
                        <th scope="col" class="yazi">Profil</th>
                        <th scope="col" class="yazi">Eklenme</br>Tarihi</th>
                        <th scope="col" class="yazi">İhale</br>Kapanış</th>
                        <th scope="col" class="yazi">Durum</th>
                        <th scope="col" class="yazi">Açıklama</th>
                        <th scope="col" class="yazi">Yayından</br>Kaldır</th>
                     </tr>
                  </thead>
                  <tbody>
                     <?php while ($ilan_oku = mysql_fetch_array($ilan_cek)) {
                        $gelen_id = $ilan_oku['id'];
                        $markayi_bul = mysql_query("SELECT * FROM marka WHERE markaID = '" . $ilan_oku['marka'] . "'");
                        $markayi_yaz = mysql_fetch_assoc($markayi_bul);
                        $durum_cek = mysql_query("SELECT * FROM kazanilan_ilanlar WHERE ilan_id = '" . $gelen_id . "'");
                        $durum_oku = mysql_fetch_array($durum_cek);
                        $durum_say = mysql_num_rows($durum_cek);
                        $durum = "";
                        $aciklama = "";
                        if ($durum_say == 1) {
                           $aciklama = $durum_oku["aciklama"];
                        } else {
                           if ($ilan_oku['durum'] == 0) {
                              $durum = "Onay Bekliyor";
                              //$aciklama = "Yönetici Onayı Bekleniyor"; 
                              $arkaplan = "yellow";
                              $color = "#000000";
                           } elseif ($ilan_oku['durum'] == 1) {
                              $durum = "Yayında";
                              //$aciklama = "İlanınız yayında"; 
                              $arkaplan = "#00b050";
                              $color = "#ffffff";
                           } elseif ($ilan_oku['durum'] == -2) {
                              $durum = "Admin İlanı Onaylamadı";
                              //$aciklama = "Yönetici Onaylamadı";
                              $arkaplan = "red";
                              $color = "#ffffff";
                           } elseif ($ilan_oku['durum'] == -3) {
                              $durum = "Admin İlanı Yayından Kaldırdı";
                              //$aciklama = "Yönetici Yayından Kaldırdı";                       
                              $arkaplan = "red";
                              $color = "#ffffff";
                           } elseif ($ilan_oku['durum'] == -4) {
                              $durum = "İlanı Yayından Kaldırdınız";
                              //$aciklama = "İlanı yayından kaldırdınız.";                       	
                              $arkaplan = "#f00";
                              $color = "#ffffff";
                           }
                        }
                        ...
                        $sil_btn = "";
                           if($ilan_oku["ihale_tarihi"]." ".$ilan_oku["ihale_saati"] > date('Y-m-d H:i:s')){
                              $sil_btn = '<a href="islemler/data_sil.php?id='.$gelen_id.'&q=ihale_sil" onclick="return confirm(\'Yayından kaldırmak istediğinize emin misiniz ?\')">
                     ...
                     ?>
                        <tr>
                           ...
                           <td class="yazi"><a href="../arac_detay.php?id=<?= $gelen_id ?>&q=ihale" target="_blank"> <?= $ilan_oku['model_yili'] . " " . $markayi_yaz['marka_adi'] . " " . $ilan_oku['model'] ?></a></td>
                           <td class="yazi"><?= $ilan_oku['sehir'] ?></td>
                           <td class="yazi"><?= $ilan_oku['profil'] ?></td>

                           <td class="yazi"><?= date("d-m-Y", strtotime($ilan_oku['eklenme_zamani'])) ?></td>
                           <td class="yazi"><?= date("d-m-Y", strtotime($ilan_oku['ihale_tarihi'] . " " . $ilan_oku['ihale_saati'])) ?></td>
                           <td class="yazi" style="background-color: <?= $arkaplan ?>!important; color:<?= $color ?>; text-transform:uppercase; text-align:center;"><?= $durum ?></td>
                           <td class="yazi"><?= $aciklama ?></td>
                           <td class="yazi">
                              <?= $sil_btn ?>
                           </td>
                        </tr>
