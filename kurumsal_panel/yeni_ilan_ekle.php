  ...
  <ul class="navbar-nav mr-auto" >
            ...
            <div class="col-sm-6">
               <form method="POST" name="form" id="form" enctype="multipart/form-data">
                  <div class="form-group">
                     ...
                     <input type="text" name="plaka" id="plaka" class="form-control" style="text-transform: uppercase;" placeholder="01AA0000" onkeypress="return boslukEngelle()" pattern="[0-9]{2}[A-Za-z]{1,3}[0-9]{2,4}" oninvalid="this.setCustomValidity('PLAKA HATALI')" oninput="this.setCustomValidity('')" maxlength="8" required />  
                  </div>
                  <div class="form-group">
                  ...
                  <input type="submit" class="btn btn-warning" onclick="myFunction()" name="ilan_ileri"   style="position: absolute; right:0; background-color :orange; color:#ffffff;" value="Ileri">
               </form>
            </div>
         </div>
      </div>
      ...
      <?php 
         if($_POST['ilan_ileri']){
				...
				var_dump("asdasdasd");
				$sorgu=mysql_query("SELECT id FROM dogrudan_satisli_ilanlar WHERE plaka='".re("plaka")."' ");
				$sorgu_ilan=mysql_query("SELECT id FROM ilanlar WHERE plaka='".re("plaka")."' ");
				$sorgu2=mysql_query("select * from ilan_uyari");
				$row=mysql_fetch_object($sorgu2);

				$ilan_uyari=$row->icerik;
				$ilan_uyari = preg_replace('/\s+/', '      ', $ilan_uyari); //çoklu boşluklar bir boşluğa çevirildi

				if(mysql_num_rows($sorgu) == 0 && mysql_num_rows($sorgu_ilan) == 0){
				
				   if($_POST['arac_durumu']=="1"){
					 ...
					 .then((value) => {
					   switch (value) {
						 case "defeat": 
							window.location.assign("ihale_ekleme_uyarilari.php?q='.re("plaka").'&d='.re("arac_durumu").'");    
								/*swal( {
										text:"'.$ilan_uyari.'",
									   buttons: {
										 defeat2: "Kabul Ediyorum",
										 cancel: "İptal",
									   },
									 }) .then((value) => {
									switch (value) { 
											case "defeat2":
											   window.location.assign("ihale_ile_ekle.php?q='.re("plaka").'&d='.re("arac_durumu").'");    
											   break;
						   ...
						   break;            
						 case "catch":
						   window.location.assign("dogrudan_satisli_ekle.php?q='.re("plaka").'&d='.re("arac_durumu").'");
						   break;
					...
					header("Location:dogrudan_satisli_ekle.php?q=".re('plaka')."&d=".re('arac_durumu'));
				}
			 }
			 else{
			   header("Location:dogrudan_satisli_ekle.php?q=".re('plaka')."&d=".re('arac_durumu'));
...
function isValid(str) {
    return !/[~`!@#$%\^&*()+=\-\[\]\\';.,/{}|\\":<>\?]/g.test(str);
      ...
      <script>
         ...
         event.target.setCustomValidity('Plaka Hatalı');
         }
      </script>
