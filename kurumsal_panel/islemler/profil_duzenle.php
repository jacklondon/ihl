...
/*	if(re("action")=="vergi_no_kontrol"){
	...
	if(re("action")=="yedek_sil"){
		...
		$guncelle=mysql_query("update user set yedek_kisi='".$yeni_yedek_ad."',yedek_kisi_tel='".$yeni_yedek_tel."' where kurumsal_user_token = '".$uye_token."'" );
		if($guncelle){
	...
	$sorgu=mysql_query("select * from user where telefon='".$onayli_cep."' and kurumsal_user_token <> '".$token."' ");
	$sorgu2=mysql_query("select * from user where sabit_tel='".$sabit_tel."' ");
	$sorgu3=mysql_query("select * from user where mail='".$email."' and kurumsal_user_token <> '".$token."' ");
	$sorgu4=mysql_query("select * from user where vergi_dairesi_no='".$vergi_no."' and kurumsal_user_token <> '".$token."' ");
	$sorgu5=mysql_query("select * from user where tc_kimlik='".$tc_kimlik."' and kurumsal_user_token <> '".$token."' ");

	$user_bul = mysql_query("select * from user where kurumsal_user_token = '".$token."'");
	$user_oku = mysql_fetch_object($user_bul);
	$user_id = $user_oku->id;
	...
	/* else if(mysql_num_rows($sorgu4)==1 && $kullaniciyi_getir_gel["vergi_dairesi_no"] != $vergi_no){
		...
		$user_guncelle=mysql_query("UPDATE user SET ad='".$ad_soyad."',tc_kimlik='".$tc_kimlik."',uye_olma_sebebi='".$sebep."',cinsiyet='".$cinsiyet."',
			...
			yedek_kisi='".$yedek_ad."',yedek_kisi_tel='".$yedek_tel."',ilce='".$ilce_adi."', vergi_dairesi = '".$vergi_dairesi."', vergi_dairesi_no = '".$vergi_no."'
			WHERE kurumsal_user_token = '".$uye_token."'");
		if($user_guncelle){
			$u=mysql_query("update uye_durumlari set hurda_teklif='".$hurda_teklif."' where uye_id='".$user_id."'");
			if($okunan_dogum_tarihi){
				$a=mysql_query("UPDATE `dogum_tarihi` SET `dogum_tarihi` = '".$dogum_tarihi."' WHERE uye_id = '".$user_id."'");
			}else{   
				mysql_query("INSERT INTO `dogum_tarihi` (`id`, `uye_id`, `dogum_tarihi`) VALUES (NULL, '".$user_id."' , '".$dogum_tarihi."')");
			}
			$response=["message"=>"Başarılı ","status"=>200];
		}
	}
