...
while($oku = mysql_fetch_array($cek)){
    $resim_cek = mysql_query("SELECT * FROM hesap_resimler WHERE hesap_id = '".$oku['id']."' ORDER BY id DESC");
 ?>
    <div class="row">
        <?= $oku['icerik'] ?>
    </div>
    <?php while($resim_oku = mysql_fetch_array($resim_cek)){ ?>
    <div class="row">
        <img src="hesap_bilgileri/<?= $resim_oku['resim']?>" style="max-width: 90%; height:auto;" alt="">
    </div>
...
<?php 
$hesap_cek = mysql_query("SELECT * FROM hesap_bilgileri ORDER BY id DESC LIMIT 1");
while($hoku = mysql_Fetch_array($hesap_cek)){ ?>
<div class="container">
 <?php  echo $hoku['icerik']; ?>
  </div>
<?php } ?>
</div>
</div>
