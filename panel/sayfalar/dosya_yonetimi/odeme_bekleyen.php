	...
	<div style="overflow-x:auto; overflow-y:auto; height:400px; margin-top:2%;">
		<table class="table table-bordered"  cellspacing="1" cellpadding="1">
			...
			<?php 
				...
				$iptalleri_cek = mysql_query("SELECT * FROM kazanilan_ilanlar WHERE durum = 1");
				while($iptalleri_oku = mysql_fetch_array($iptalleri_cek)){
					...
					$pd_hizmet=$iptalleri_oku["pd_hizmet"];
					$dosya_masrafi=$iptalleri_oku["dosya_masrafi"];
					$statu_mtv=$iptalleri_oku["mtv"];
					$statu_mtv_not=$iptalleri_oku["mtv_not"];

					if($iptalleri_oku["parca_1"]!=""){
						$statu_parca_1=$iptalleri_oku["parca_1"]."₺";
					}else{
						$statu_parca_1="";
					}
					if($iptalleri_oku["parca_1"]!=""){
						$statu_parca_2=$iptalleri_oku["parca_2"]."₺";
					}else{
						$statu_parca_2="";
					}
					if($iptalleri_oku["parca_1"]!=""){
						$statu_parca_3=$iptalleri_oku["parca_3"]."₺";
					}else{
						$statu_parca_3="";
					}
					
					$statu_parca_1_not=$iptalleri_oku["parca_1_not"];
					$statu_parca_2_not=$iptalleri_oku["parca_2_not"];
					$statu_parca_3_not=$iptalleri_oku["parca_3_not"];
					...
					if($hepsini_oku["profil"]=="Hurda Belgeli"){
						$noter_ucreti="Noter devri esnasında hesaplanıcak";
						$toplam_ucret=$iptalleri_oku["kazanilan_teklif"]+$iptalleri_oku["dosya_masrafi"]+$iptalleri_oku["pd_hizmet"];
					}else{
						$noter_ucreti=money($iptalleri_oku["noter_takipci_gideri"])."₺";
			...
			<tr>
				<?php 
					...
					$marka_adi = $marka_oku['marka_adi'];
					$kazanan_kisi_cek = mysql_query("SELECT * FROM user WHERE id = '".$iptalleri_oku['uye_id']."' LIMIT 1"); ?>
					<td><a target="_blank" href="?modul=ilanlar&sayfa=ilan_ekle&id=<?= $iptal_id ?>"><i class="fas fa-edit"></i></a></td>
					<td><a target="_blank" href="?modul=ilanlar&sayfa=ilan_resim_ekle&id=<?= $iptal_id ?>"><i class="far fa-image"></i></a></td>
					<td><?= $hepsini_oku["arac_kodu"] ?> </td>
					<td><?=$hepsini_oku["plaka"]?></td>
					<td><a target="_blank" href="../arac_detay.php?id=<?=$iptal_id?>&q=ihale" style="text-decoration: none; color:#000000; cursor:pointer;"> 
						<?=$hepsini_oku["model_yili"]. " " .$marka_adi ." ". $hepsini_oku['model']." ". $hepsini_oku['tip'] ?></a>
					</td>
					...
					while($kazanan_kisi_oku = mysql_fetch_array($kazanan_kisi_cek)){ 
						...
						<td><?=$noter_ucreti ?></td>
						<td class="<?=$son_odeme_class  ?>" ><?=$son_odeme ?></td>
						<td><a class="view_ilan_teklifleri" id="ilanteklif_<?= $iptal_id ?>"><i class="fas fa-gavel"><?=$teklif_sayi ?></i></a></td> 
						<td><a class="view_ilan_favorileri" id="<?= $iptal_id ?>"><i class="fas fa-heart"><?=$favori_sayi ?></i></a></td>  
						<td><a class="view_ilan_mesajlari" id="ilanmesaj_<?= $iptal_id ?>"><i class="fas fa-envelope"><?=$mesaj_sayi ?></i></a></td>   
						<td><a class="view_ilan_notlari" id="<?= $iptal_id ?>"><i class="fas fa-align-justify"><?=$not_sayi ?></i></a></td> 
...
<script>
	function filtre_cikar(key,value){
		...
		if(key=="aranan_plaka" && value=="filtre"){
			$("#aranan_plaka").val("");
		}else if(key=="aranan_arac_kodu" && value=="filtre"){
			$("#aranan_arac_kodu").val("");
		}else{
