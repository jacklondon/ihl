...
if (!isset($_GET['startrow']) or !is_numeric( $_GET['startrow'] ) ) {
	...
	$total_rows = mysql_num_rows(mysql_query("SELECT * FROM ilanlar WHERE ihale_tarihi = '$yarin' and durum=1"));
	...
	// $hepsini_cek = mysql_query("SELECT * FROM ilanlar WHERE ihale_tarihi = '$yarin' and durum=1 ORDER BY concat(ihale_tarihi,' ',ihale_saati) asc ".$limit_sql);
	$hepsini_cek = mysql_query("SELECT * FROM ilanlar WHERE ihale_tarihi = '$yarin' and durum=1 ORDER BY concat(ihale_tarihi,' ',ihale_saati) asc ");
	//var_dump("SELECT * FROM ilanlar WHERE ihale_tarihi = '$yarin' and durum=1 ORDER BY concat(ihale_tarihi,' ',ihale_saati) asc");
	$filtreler="";
?>
...
if(isset($_POST['filtrele'])){    
	...
	<form method="POST" action="?modul=ihaleler&sayfa=toplu_sil" onsubmit="return confirm('SEÇİLİ İLANLARI SİLMEK İSTEDİĞİNİZE EMİN MİSİNİ?');" >
		...
		<div style="overflow-x:auto; margin-top:4%;">
			<table id="yarin_bitenler_table" class="table table-bordered" cellspacing="1" cellpadding="1">
				<thead>
				<tr>
					<td><input type="checkbox" id="checkle" class="checkall btn btn-blue chec2" style="padding:20px;opacity:1!important; z-index:999;">Seç</td>
					<td>Düzenle</td>
