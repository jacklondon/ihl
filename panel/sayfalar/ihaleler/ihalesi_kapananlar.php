...
if (!isset($_GET['startrow']) or !is_numeric( $_GET['startrow'] ) ) {
	...
	$total_rows = mysql_num_rows(mysql_query("SELECT * FROM ilanlar WHERE concat(ihale_tarihi, ' ', ihale_saati) <= '".date('Y-m-d H:i:s')."' and durum=-1"));
	...
	// $hepsini_cek = mysql_query("SELECT * FROM ilanlar WHERE concat(ihale_tarihi, ' ', ihale_saati) <= '".date('Y-m-d H:i:s')."' and durum=-1 ORDER BY ihale_tarihi asc ".$limit_sql); 
	// $hepsini_cek = mysql_query("SELECT * FROM ilanlar WHERE concat(ihale_tarihi, ' ', ihale_saati) <= '".date('Y-m-d H:i:s')."' and durum=-1 ORDER BY ihale_tarihi asc "); 
	$hepsini_cek = mysql_query("SELECT * FROM ilanlar WHERE concat(ihale_tarihi, ' ', ihale_saati) <= '".date('Y-m-d H:i:s')."' and durum=-1  ORDER BY ihale_tarihi DESC, ihale_saati DESC "); 
...
if(isset($_POST['filtrele'])){    
	...
	<form method="POST" action="?modul=ihaleler&sayfa=toplu_sil" onsubmit="return confirm('SEÇİLİ İLANLARI SİLMEK İSTEDİĞİNİZE EMİN MİSİNİ?');" >
		...
		<div style="overflow-x:auto; margin-top:4%;">
			<table id="ihalesi_bitenler_table" class="table table-bordered" cellspacing="1" cellpadding="1">
				<thead>
				<tr>
					<td><input type="checkbox" id="checkle" class="checkall btn btn-blue chec2" style="padding:20px;opacity:1!important; z-index:999;">Seç</td>
					<td>Düzenle</td>
