<?php 
	...
	ilanlar.*,t.teklif_zamani,t.ilan_id,t.durum as teklif_durum,t.uye_id
FROM 
	ilanlar
INNER JOIN 
	(
		...
		WHERE
			teklif_zamani=(
				...
				FROM 
					teklifler as t2
				WHERE
					t1.ilan_id=t2.ilan_id and t2.durum=1
				   order by 
					teklif_zamani  desc
				limit 1
				) 
			AND 
				t1.uye_id!='283' and t1.durum=1
		GROUP BY
			ilan_id
		ORDER BY
			teklif_zamani DESC
...
ON
	ilanlar.id = t.ilan_id
WHERE 
	ilanlar.durum=-1 and ilanlar.ihale_turu=1"));
...
/*
	$hepsini_cek = mysql_query("
		...
		ON
			ilanlar.id = t.ilan_id
		WHERE 
			ilanlar.durum=-1 and ilanlar.ihale_turu=1 ".$limit_sql);
			*/
	$hepsini_cek = mysql_query("
		SELECT 
			ilanlar.*,t.teklif_zamani,t.ilan_id,t.durum as teklif_durum,t.uye_id
		...
		ON
			ilanlar.id = t.ilan_id
		WHERE 
			ilanlar.durum=-1 and ilanlar.ihale_turu=1 ORDER BY ilanlar.ihale_tarihi DESC, ilanlar.ihale_saati DESC");
	$filtreler="";
?>
...
if(isset($_POST['filtrele'])){    
	...
	<form method="POST" action="?modul=ihaleler&sayfa=toplu_sil" onsubmit="return confirm('SEÇİLİ İLANLARI SİLMEK İSTEDİĞİNİZE EMİN MİSİNİ?');" >
		...
		<div style="overflow-x:auto; margin-top:4%;">
			<table id="statu_bekleyenler_table" class="table table-bordered" cellspacing="1" cellpadding="1">
				<thead>
				<tr>
					<td><input type="checkbox" id="checkle" class="checkall btn btn-blue chec2" style="padding:20px;opacity:1!important; z-index:999;">Seç</td>
					<td>Düzenle</td>
		...
		var x = setInterval(function() 
		{
			...
			url: "https://ihale.pertdunyasi.com/check.php",
				...
				success: function(response) {
					...
					//$("#tr_"+id).css("background-color","#00a2ff");
