...
if (!isset($_GET['startrow']) or !is_numeric( $_GET['startrow'] ) ) {
	...
	$total_rows = mysql_num_rows(mysql_query("SELECT ilanlar.*,t.teklif_zamani FROM ilanlar INNER JOIN (SELECT t1.* FROM teklifler as t1 WHERE t1.durum=1 and t1.teklif_zamani=(SELECT t2.teklif_zamani FROM teklifler as t2 WHERE t1.ilan_id=t2.ilan_id 
	ORDER BY t2.teklif_zamani DESC LIMIT 1) ) as t ON ilanlar.id = t.ilan_id and t.durum = 1 ORDER BY t.teklif_zamani DESC "));
	...
	$hepsini_cek=mysql_query("SELECT ilanlar.*,t.teklif_zamani FROM ilanlar INNER JOIN (SELECT t1.* FROM teklifler as t1 WHERE t1.durum=1 and t1.teklif_zamani=(SELECT t2.teklif_zamani FROM teklifler as t2 WHERE t1.ilan_id=t2.ilan_id ORDER BY 
	t2.teklif_zamani DESC LIMIT 1 ) ) as t ON ilanlar.id = t.ilan_id and t.durum = 1 ORDER BY t.teklif_zamani DESC ");	
	$filtreler="";
?>
...
if(isset($_POST['filtrele'])){    
	...
	<form method="POST" action="?modul=ihaleler&sayfa=toplu_sil" onsubmit="return confirm('SEÇİLİ İLANLARI SİLMEK İSTEDİĞİNİZE EMİN MİSİNİ?');" >
		...
		<div style="overflow-x:auto; margin-top:4%;">
			<table id="son_teklifler_table" class="table table-bordered" cellspacing="1" cellpadding="1">
				<thead>
				<tr>
					<th><input type="checkbox" id="checkle" class="checkall btn btn-blue chec2" style="padding:20px;opacity:1!important; z-index:999;">Seç</th>
					<th>Düzenle</th>
					<th>Görseller</th>
					<th>Kod</th>
					<th>Plaka</t>
					...
					while($hepsini_oku = mysql_fetch_array($hepsini_cek)){
						...
						if($toplam_teklif>0){
							if($statu_oku['durum'] == "0" || $statu_oku['durum'] == "1" || $statu_oku['durum'] == "2" || $statu_oku['durum'] == "3" || $statu_oku['durum'] == "4"){
								...
								$srgl=mysql_query("select ilanlar.* from ilanlar inner join teklifler on teklifler.ilan_id=ilanlar.id  where ilanlar.durum=1 and ilanlar.ihale_tarihi='".$ihale_trh."' and ilanlar.ihale_saati='".$ihale_st."' and teklifler.durum=1 and ilanlar.id = '".$hepsini_oku["id"]."' group by ilanlar.id");			
								...
								// var_dump("select ilanlar.* from ilanlar inner join teklifler on teklifler.ilan_id=ilanlar.id  where ilanlar.durum=1 and ilanlar.ihale_tarihi='".$ihale_trh."' and ilanlar.ihale_saati='".$ihale_st."' and teklifler.durum=1 group by ilanlar.id");
								if(mysql_num_rows($srgl)>0){
									while($oku=mysql_fetch_object($srgl)){
										if(!in_array($oku->id,$hepsi_array)){
											...
											$td.='<tr id="tr_'.$oku->id.'" style="background-color: '.$tablo_rengi.'; '.$t_color.' ">
													<td><input type="checkbox" name="secim[]" class="chec" id="asd'.$oku->id.'" value="'.$oku->id.'" style="opacity:1!important; z-index:999;"></td>
													<td><a target="_blank" href="?modul=ilanlar&sayfa=ilan_ekle&id='.$oku->id.'"><i class="fas fa-edit"></i></a></td>
													<td><a onclick="tabTrigger()" href="?modul=ilanlar&sayfa=ilan_ekle&id='.$oku->id.'" target="_blank"><img style="width: 50px; height:50px;" src="../images/'.$resim.'" alt=""></a></td>
													<td>'.$oku->arac_kodu.'</td>
													...
													<td class="'.$yan.'"><a class="view_uyeye_teklif_ver" style="font-size:17px;'.$t_color.'" id="sonteklif_'.$oku->id.'">'.money($oku->son_teklif).'₺</a></td>   
													<!-- <td class="'.$yan.'">'.$oku->son_teklif.'</td>     -->
													<input type="hidden" id="teklif_sayisi'.$sira.'" value="'.$toplam_teklif.'" >
													<td><a style="'.$t_color.'" class="view_ilan_teklifleri" id="teklifler_'.$oku->id.'"><i class="fas fa-gavel">'.$toplam_teklif.'</i></a></td>    
													<td><a style="'.$t_color.'" class="view_ilan_mesajlari" id="mesajlar_'.$oku->id.'"><i class="fas fa-envelope">'.$mesaj_sayi.'</i></a></td>    
									...
									if(!in_array($hepsini_oku['id'],$hepsi_array)){
										...
										$td.='
											<tr id="tr_'.$hepsini_oku["id"].'" style="background-color: '.$tablo_rengi.';'.$t_color.'  ">
												...
												<td class="'.$yan.'"><a style="font-size:17px;'.$t_color.'" class="view_uyeye_teklif_ver" id="sonteklif_'.$hepsini_oku['id'].'">'.money($hepsini_oku["son_teklif"]).'₺</a></td>   
							...
							$td.='
								<tr id="tr_'.$hepsini_oku["id"].'"  style="background-color: '.$tablo_rengi.'; ">
									...
									<td class="'.$yan.'"><a class="view_uyeye_teklif_ver" style="font-size:17px;" id="sonteklif_'.$hepsini_oku['id'].'">'.money($hepsini_oku["son_teklif"]).'₺</a></td>   
									<!-- <td class="'.$yan.'">'.$hepsini_oku["son_teklif"].'</td>     -->
									<input type="hidden" id="teklif_sayisi'.$sira.'" value="'.$toplam_teklif.'" >
