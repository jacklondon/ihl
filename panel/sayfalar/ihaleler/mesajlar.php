<?php 
	...
	$hepsini_cek = mysql_query("select ilanlar.* from ilanlar inner join chat_room on chat_room.ilan_id=ilanlar.id group by ilanlar.id order by chat_room.last_message_time desc");
?>
	...
	<form method="POST" action="?modul=ihaleler&sayfa=toplu_sil" onsubmit="return confirm('SEÇİLİ İLANLARI SİLMEK İSTEDİĞİNİZE EMİN MİSİNİ?');" >
		...
		<div style="overflow-x:auto; margin-top:4%;">
			<table id="mesajlar_ilan_table" class="table table-bordered" cellspacing="1" cellpadding="1">
				...
				<?php 
				// $sira=0; $ihale_say=mysql_num_rows($hepsini_cek);
				$sira=0; $ihale_say=mysql_num_rows(mysql_query("select * from chat_room where status = 1 order by last_message_time desc"));
				?>
					...
					$krem_array=array(); 
					$hepsi_array=array();
					$mesajlarini_cek_yeni = mysql_query("select * from chat_room where status = 1 order by last_message_time desc");
					while($mesajlarini_oku_yeni = mysql_fetch_object($mesajlarini_cek_yeni)){
						$ilan_id = $mesajlarini_oku_yeni->ilan_id;
