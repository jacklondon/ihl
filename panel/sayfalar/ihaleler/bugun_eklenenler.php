...
if (!isset($_GET['startrow']) or !is_numeric( $_GET['startrow'] ) ) {
	...
	$total_rows = mysql_num_rows(mysql_query("SELECT * FROM ilanlar WHERE eklenme_zamani = '".$bugun."'"));
	...
	// $hepsini_cek = mysql_query("SELECT * FROM ilanlar WHERE eklenme_zamani = '".$bugun."' ORDER BY id asc ".$limit_sql); 
	// $hepsini_cek = mysql_query("SELECT * FROM ilanlar WHERE eklenme_zamani = '".$bugun."' ORDER BY id asc "); 
	$hepsini_cek = mysql_query("SELECT * FROM `ilanlar` WHERE DATE(`eklenme_zamani`) = CURDATE() ORDER BY eklenme_zamani DESC "); 
	$filtreler="";
?>
...
if(isset($_POST['filtrele'])){    
	...
	<form method="POST" action="?modul=ihaleler&sayfa=toplu_sil" onsubmit="return confirm('SEÇİLİ İLANLARI SİLMEK İSTEDİĞİNİZE EMİN MİSİNİ?');" >
		...
		<div style="overflow-x:auto; margin-top:4%;">
			<table id="bugun_eklenenler_table" class="table table-bordered" cellspacing="1" cellpadding="1">
				<thead>
					<tr>
						<td><input type="checkbox" id="checkle" class="checkall btn btn-blue chec2" style="padding:20px;opacity:1!important; z-index:999;">Seç</td>
						<td>Düzenle</td>
				...
				<?php $sira=0; $ihale_say=mysql_num_rows($hepsini_cek);?>
					...
					while($hepsini_oku = mysql_fetch_array($hepsini_cek)){
						...
						$ilan_sahibi_admin_cek = mysql_query("select * from kullanicilar where token = '".$hepsini_oku["ihale_sahibi"]."'");
						if(mysql_num_rows($ilan_sahibi_admin_cek) != 0){
							$ilan_sahibi_admin_oku = mysql_fetch_object($ilan_sahibi_admin_cek);
							$ilan_sahibi = $ilan_sahibi_admin_oku->adi." ".$ilan_sahibi_admin_oku->soyadi;
						}else{
							$ilan_sahibi_bireysel_cek = mysql_query("select * from user where user_token = '".$hepsini_oku["ihale_sahibi"]."'");
							if(mysql_num_rows($ilan_sahibi_bireysel_cek) != 0){
								$ilan_sahibi_bireysel_oku = mysql_fetch_object($ilan_sahibi_bireysel_cek);
								$ilan_sahibi = $ilan_sahibi_bireysel_oku->ad;
							}else{
								$ilan_sahibi_kurumsal_cek = mysql_query("select * from user where kurumsal_user_token = '".$hepsini_oku["ihale_sahibi"]."'");
								if(mysql_num_rows($ilan_sahibi_kurumsal_cek) != 0){
									$ilan_sahibi_kurumsal_oku = mysql_fetch_object($ilan_sahibi_kurumsal_cek);
									$ilan_sahibi = $ilan_sahibi_kurumsal_oku->unvan;
								}else{
									$ilan_sahibi = "";
								}
							}
