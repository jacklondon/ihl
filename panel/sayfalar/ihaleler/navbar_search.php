<?php 
	...
	$hepsini_cek = mysql_query("SELECT * FROM ilanlar where  concat(plaka,arac_kodu) LIKE '%".re("navbar_aranan")."%' ORDER BY concat(ihale_tarihi,' ',ihale_saati) asc ");
?>
<?php 
	...
	$admin_id = $_SESSION['kid'];
	//$hepsini_cek = mysql_query("SELECT * FROM ilanlar where durum=1 ORDER BY concat(ihale_tarihi,' ',ihale_saati) asc");
	// $hepsini_cek = mysql_query("SELECT * FROM ilanlar where ihale_son_gosterilme>'".date("Y-m-d H:i:s")."' ORDER BY concat(ihale_tarihi,' ',ihale_saati) asc LIMIT $startrow, 100");
	$limit = 100;
	$page = (re("page") != "" && is_numeric(re("page"))) ? re("page") : 1;	
	$total_rows = mysql_num_rows(mysql_query("SELECT * FROM ilanlar where  concat(plaka,arac_kodu) LIKE '%".re("navbar_aranan")."%' ORDER BY concat(ihale_tarihi,' ',ihale_saati) asc "));
			...
			<table id="tum_ihaleler_table" class="table table-bordered" cellspacing="1" cellpadding="1">
				...
				<?php $sira=0; $ihale_say=mysql_num_rows($hepsini_cek);?>
					...
					while($hepsini_oku = mysql_fetch_array($hepsini_cek)){
						...
						if($hepsini_oku["ihale_son_gosterilme"] > date("Y-m-d H:i:s")){
							$ilan_yeni_durum = 1;
						}else{
							if($toplam_teklif == 0){
								$ilan_yeni_durum = 0;
