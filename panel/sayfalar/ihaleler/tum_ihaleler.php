<?php 
	...
	$total_rows = mysql_num_rows(mysql_query("SELECT * FROM ilanlar WHERE durum = 1 ORDER BY concat(ihale_tarihi,' ',ihale_saati) asc "));
	$total_page = ceil($total_rows / $limit);
	$page = ($page < 1) ? 1 : $page;
	$page = ($page > $total_page) ? $total_page : $page;
	$limit_start = ($page-1)*$limit;
	$limit_sql = "LIMIT ".$limit_start.",".$limit;
	// $hepsini_cek = mysql_query("SELECT * FROM ilanlar WHERE durum = 1 ORDER BY concat(ihale_tarihi,' ',ihale_saati) asc ".$limit_sql);
	$hepsini_cek = mysql_query("SELECT * FROM ilanlar WHERE durum = 1 ORDER BY concat(ihale_tarihi,' ',ihale_saati) asc");
...
if(isset($_POST['filtrele'])){    
	...
	if($f_arama_kriteri!="" && $f_kucuk_kapanis =="" && $f_buyuk_kapanis =="" && $f_kucuk_ekleme =="" && $f_buyuk_ekleme ==""){
		if(re('kucuk_ekleme') =="" && re('buyuk_ekleme') =="" && re('kucuk_kapanis') =="" && re('buyuk_kapanis') ==""){
			...
			if($seciliAramaKriteriSayisi != 0 && $arama_where != ""){
