...
function uyeler_telefon_formatla($telefon){
	...
	$uyeleri_cek = mysql_query("SELECT user.*,onayli_kullanicilar.id as o_id FROM `user` inner join onayli_kullanicilar on user.id=onayli_kullanicilar.user_id where 
	onayli_kullanicilar.durum=1 and user.temsilci_id='".$admin_id."' order by id desc");
	// var_dump("SELECT user.*,onayli_kullanicilar.id as o_id FROM `user` inner join onayli_kullanicilar on user.id=onayli_kullanicilar.user_id where 
	// onayli_kullanicilar.durum=1 and user.temsilci_id='".$admin_id."' order by id desc");
...
?>
	...
	$filtre_cek = "SELECT u.* FROM user as u inner join onayli_kullanicilar as o_k on u.id=o_k.user_id inner join uye_durumlari as u_d on u.id=u_d.uye_id $where and o_k.durum=1 and u.temsilci_id='".$admin_id."'";
		...
		if($sayi==0){
			...
			echo 'window.location.href = "?modul=bana_ozel&sayfa=uyelerim";';
			echo '</script>';  
			
		}else{ 
			echo $sayi . "Adet sonuç listelendi." ?>
			<?php if($filtre_var=="true"){ ?>
				<p><a href="?modul=bana_ozel&sayfa=uyelerim">Tümünü Temizle</a></p>
			<?php } ?>
