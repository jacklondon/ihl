<?php 
function uyeler_telefon_formatla($telefon){
	$bir = str_replace("(","",$telefon);
	$iki = str_replace(")","",$bir);
	$sonuc = str_replace("-","",$iki);
	return $sonuc;
}
	session_start();
	...
	$total_rows = mysql_num_rows(mysql_query("SELECT user.*,onayli_kullanicilar.id as o_id FROM `user` inner join onayli_kullanicilar on user.id=onayli_kullanicilar.user_id where onayli_kullanicilar.durum=1"));
	$total_page = ceil($total_rows / $limit);
	$page = ($page < 1) ? 1 : $page;
	$page = ($page > $total_page) ? $total_page : $page;
	$limit_start = ($page-1)*$limit;
	$limit_sql = "LIMIT ".$limit_start.",".$limit;
	// $uyeleri_cek = mysql_query("SELECT user.*,onayli_kullanicilar.id as o_id FROM `user` inner join onayli_kullanicilar on user.id=onayli_kullanicilar.user_id where onayli_kullanicilar.durum=1 order by id desc ".$limit_sql);
...
?>
		...
		<div class="row-fluid" style="margin-top: 2%;">
			<div class="span6">
				...
				<?php if($_POST["aranan_tel"]!="" ){?>
					<input style="width: calc(100% - 35px);" type="tel" id="aranantel" name="aranan_tel" value="<?=$_POST["aranan_tel"] ?>" class="span12">
				<?php }else{ ?>
					<input style="width: calc(100% - 35px);" type="tel" id="aranantel" name="aranan_tel" class="span12">
				<?php } ?>
				<a onclick="whatsappa_git(document.getElementById('aranantel').value)"><i class="fab fa-whatsapp"></i></a>
				...
				<script>
					function whatsappa_git(telefon){
						var bir = telefon.replace("(", "");
						var iki = bir.replace(")", "");
						var sonuc = iki.replace("-", "");
						window.open('https://wa.me/9'+sonuc);
					}
				</script>
				<div class="multiselect">
				...
				<div class="multiselect">
					...
					<div id="adminler">
						...
						<?php }else{
							while($admin_oku = mysql_fetch_array($admin_cek)){?>                        
								<label for="<?= $admin_oku['id'] ?>">
								<input type="checkbox" name="admin[]" id="admin_<?= $admin_oku['id'] ?>" value="<?= $admin_oku['id'] ?>" /><?= $admin_oku['adi']." ".$admin_oku['soyadi'] ?>
							</label>
							<?php } 
						}
					?>
	...
	<?php 
		...
		$f_ilk_tarih =$_POST['kayit1'] ;
		$f_ikinci_tarih =$_POST['kayit2'] ;
		...
		if($f_ilk_tarih != "" && $f_ikinci_tarih != ""){
			// if($f_ilk_tarih == $f_ikinci_tarih){
			// 	$where .= " AND DATE(kayit_tarihi) = '".$f_ilk_tarih."' ";				
			// }else{
			// 	$where .= " AND  kayit_tarihi BETWEEN '".$f_ilk_tarih."' AND '".$f_ikinci_tarih."' ";
			// }
			// $where .= " AND  kayit_tarihi BETWEEN '".$f_ilk_tarih."' AND '".$f_ikinci_tarih."' ";
			$where .= " AND DATE(kayit_tarihi) >= '".$f_ilk_tarih."' AND DATE(kayit_tarihi) <= '".$f_ikinci_tarih."'";
			$onclick='';
			$filtre_var="true";
			$onclick='onclick="filtre_cikar(\'kucukbuyuk_\',\'kayit\')";';
			$filtreler.='
				<div class="filter_box">
					Kayıt tarih: '.date("d-m-Y",strtotime($f_ilk_tarih)).' - '.date("d-m-Y",strtotime($f_ikinci_tarih)).'
					<div '.$onclick.' class="filter_box_close">
						x
					</div>
				</div>	
			';
		}elseif($f_ilk_tarih != "" && $f_ikinci_tarih == ""){
			// $where .= " AND  kayit_tarihi >= '".$f_ilk_tarih."' ";
			$where .= " AND DATE(kayit_tarihi) >= '".$f_ilk_tarih."'";
			$onclick='';
			$filtre_var="true";
			$onclick='onclick="filtre_cikar(\'kucukbuyuk_\',\'kayit\')";';
			$filtreler.='
				<div class="filter_box">
					Kayıt tarih: '.date("d-m-Y",strtotime($f_ilk_tarih)).' - 
					<div '.$onclick.' class="filter_box_close">
						x
					</div>
				</div>	
			...
			// $where .= " AND  kayit_tarihi <= '".$f_ikinci_tarih."' ";
			$where .= " AND DATE(kayit_tarihi) <= '".$f_ikinci_tarih."'";
			$onclick='';
			$filtre_var="true";
			$onclick='onclick="filtre_cikar(\'kucukbuyuk_\',\'kayit\')";';
			$filtreler.='
				<div class="filter_box">
					Kayıt tarih: - '.date("d-m-Y",strtotime($f_ikinci_tarih)).'
					<div '.$onclick.' class="filter_box_close">
						x
					</div>
				</div>	
		...
		*/
		...
		if($f_giris1 != "" && $f_giris2 != ""){
			if($f_giris1 == $f_giris2){
				$where .= " AND DATE(son_islem_zamani) = '".$f_giris1."' ";
			}else{
				$where .= " AND  son_islem_zamani BETWEEN '".$f_giris1."' AND '".$f_giris2."'";
			}
			$onclick='';
			$filtre_var="true";
			...
			$filtreler.='
				<div class="filter_box">
					Son Giriş tarih: '.date("d-m-Y",strtotime($f_giris1)).' - '.date("d-m-Y",strtotime($f_giris2)).'
					<div '.$onclick.' class="filter_box_close">
						x
					</div>
				</div>	
			';
		}elseif($f_giris1 != "" && $f_giris2 == ""){
			$where .= " AND son_islem_zamani >= '".$f_giris1."' ";
			$onclick='';
			$filtre_var="true";
			$onclick='onclick="filtre_cikar(\'kucukbuyuk_\',\'giris\')";';
			$filtreler.='
				<div class="filter_box">
					Son Giriş tarih: '.date("d-m-Y",strtotime($f_giris1)).' - 
					<div '.$onclick.' class="filter_box_close">
						x
					</div>
				</div>	
			...
			$where .= " AND son_islem_zamani <= '".$f_giris2."' ";
			$onclick='';
			$filtre_var="true";
			$onclick='onclick="filtre_cikar(\'kucukbuyuk_\',\'giris\')";';
			$filtreler.='
				<div class="filter_box">
					Son Giriş tarih:  - '.date("d-m-Y",strtotime($f_giris2)).'
					<div '.$onclick.' class="filter_box_close">
						x
					</div>
				</div>	
		...
		*/
		...
		if($f_admin !=""){
			...
			$seciliAdminSayisi = count($_POST['admin']);
			$seciliAdmin = "";
			while ($ai < $seciliAdminSayisi) {
				$admin_read=mysql_query("select * from kullanicilar where id='".$_POST["admin"][$ai]."'");
				$admin_fetch=mysql_fetch_assoc($admin_read);
				$onclick='';
				$onclick='onclick="filtre_cikar(\'admin_\',\''.$_POST["admin"][$ai].'\')"';
				$filtreler.='
					<div class="filter_box">
				...
				$seciliAdmin = $seciliAdmin . "'" . $_POST['admin'][$ai] . "'";
				if ($ai < $seciliAdminSayisi - 1) {
			...
			while ($d < $seciliDurumSayisi) {
				...
				*/
				...
				if($_POST['uye_durum'][$d]=="1"){
					$seciliDurum.=" and u_d.teklif_engelle <> 'on' and u_d.uyelik_iptal <> 'on' and u_d.kalici_mesaj <> 'on' ";
		...
		if($f_uye_ilgi !=""){
			...
			while ($ui < $seciliİlgiSayisi) {
				...
				*/
				$onclick='';
				$secili_ilgi_text="";
				if($_POST['uye_ilgi'][$ui]=="1"){
					if($ui!=$seciliİlgiSayisi-1){
				...
				}else if($_POST['uye_ilgi'][$ui]=="9"){
					if($ui!=$seciliİlgiSayisi){
				...
				$onclick='onclick="filtre_cikar(\'ilgi_\',\''.$_POST['uye_ilgi'][$ui].'\')"';
				$filtreler.='
					<div class="filter_box">
						'.$secili_ilgi_text.'
						<div '.$onclick.' class="filter_box_close">
		...
		if($sayi==0){
			...
			<?php if($filtre_var=="true"){ ?>
				<p><a href="?modul=uyeler&sayfa=uyeler">Tümünü Temizle</a></p>
			<?php } ?>
		...
		<form method="POST" action="?modul=uyeler&sayfa=uyeler_islem">
			...
			<div style="overflow-x:auto; overflow-y:auto;  margin-top:2%; padding: 25px;">
				<table id="dene_table_ugur" width="100%" class="table table-bordered">
					<thead>
						<tr>
							<th>Seç</th>
							<th>Durumu</th>
					...
					<tbody>
						...
						<tr>
							...
							<td><?= $filtre_oku["ad"] ?></td>
							<td>
								<a href="?modul=uyeler&sayfa=sms_gonder&id=<?=$filtre_oku["id"]?>" target="_blank"><?= $filtre_oku["telefon"] ?></a>
								<a target="_blank" href="https://wa.me/9<?= uyeler_telefon_formatla($filtre_oku["telefon"]) ?>"><i class="fab fa-whatsapp"></i></a>
							</td>
							...
							<td><a id="<?= $filtre_oku['id']?>" class="view_teklifleri"><i class="fas fa-gavel"><?= $aktif_teklif_say ?></i></a></td>
							...
							<td id="td_view_notlari_<?=$filtre_oku["id"] ?>"  ><a id="<?= $filtre_oku['id'] ?>" class="view_notlari"><i class="fas fa-align-left"><?= $prm_say+$evrak_say+$yuklenenler  ?></i></a></td>
							...
							<td class="dropdown">
								...
								<ul class="dropdown-menu"style="position: absolute; transform: translate3d(90px, 10px, 0px); top: -30px; left: -240px; will-change: transform;">
									<li><a href="javascript:void(0)" onclick="sifre_yenile_sms(<?= $filtre_oku['id'] ?>);">Şifre Hatırlat (Sms)</a></li>
									<li><a href="javascript:void(0)" onclick="sifre_yenile(<?=$filtre_oku['id'] ?>); alert('Mail gönderildi'); ">Şifre Hatırlat (Eposta)</a></li>
									<li><a href="javascript:void(0)" onclick="tanitim_sms_gonder(<?= $filtre_oku['id'] ?>);" >Tanıtım Mesajı Gönder</a></li>
									<li><a href="javascript:void(0)" onclick="prm_ekle(<?= $filtre_oku['id'] ?>);">PRM notu ekle</a></li>
						...
						<input aria-hidden="true" type="text" class="offscreen" id="kopyalanan_filtre<?=$count ?>" value="
							Alıcı Adı : <?= $uye_isim_bilgi ?> Kargo Adresi : <?= $filtre_oku['adres']." ".$filtre_oku['ilce']." ".$filtre_oku['sehir'] ?> İletişim : <?= $filtre_oku['telefon'] ?>" />
		...
		<form method="POST" action="?modul=uyeler&sayfa=uyeler_islem" name="form" id="form">
			...
			<div style="overflow-x:auto; overflow-y:auto;  margin-top:4%; padding: 25px;">
				<table id="uyeler_table" width="100%" class="table table-bordered" cellspacing="2">
					<thead>
						<tr>
					...
					<tbody>
						...
						while($uyeleri_oku = mysql_fetch_array($uyeleri_cek)){ 
							...
							<tr>
								...
								<td>
									<a href="?modul=uyeler&sayfa=sms_gonder&id=<?=$uyeleri_oku["id"]?>" target="_blank"><?= $uyeleri_oku["telefon"] ?></a>
									<a target="_blank" href="https://wa.me/9<?= uyeler_telefon_formatla($uyeleri_oku["telefon"]) ?>"><i class="fab fa-whatsapp"></i></a>
								</td>
								...
								<td id="td_view_notlari_<?=$uyeleri_oku["id"] ?>" ><a id="<?= $uyeleri_oku['id'] ?>" class="view_notlari"><i class="fas fa-align-left"><?= $prm_say+$evrak_say+$yuklenenler ?></i></a></td>
								...
								<td class="dropdown">
									<a class="dropwn-toggle" data-toggle="dropdown"><i class="fas fa-align-justify"></i></a>
									<ul class="dropdown-menu"style="position: absolute; transform: translate3d(90px, 10px, 0px); top: -30px; left: -240px; will-change: transform;">
										<li><a 	href="javascript:void(0)" onclick="sifre_yenile_sms(<?= $uyeleri_oku['id'] ?>);">Şifre Hatırlat (Sms)</a></li>
										<li><a 	href="javascript:void(0)" onclick="sifre_yenile(<?=$uyeleri_oku['id'] ?>);alert('Mail gönderildi');">Şifre Hatırlat (Eposta)</a></li>
										<li><a  href="javascript:void(0)" onclick="tanitim_sms_gonder(<?= $uyeleri_oku["id"] ?>);" >Tanıtım Mesajı Gönder</a></li>
										<li><a  href="javascript:void(0)" onclick="prm_ekle(<?= $gelen_id ?>);">PRM notu ekle</a></li>
										<!-- <li><a href="?modul=ayarlar&sayfa=arac_detay_musteri_temsilcisi">Araç detay önemli metni</a></li> -->
										<!-- <li><a href="?modul=uyeler&sayfa=temsilci_ekle&id=<?= $gelen_id ?>">Temsilci Ata</a></li> -->
									</ul>
								</td>
							...
							<input aria-hidden="true" type="text" class="offscreen" id="kopyalanan<?=$sira ?>" 
								value=" 				
									Alıcı Adı : <?= $uye_isim_bilgi ?> Kargo Adresi : <?= $uyeleri_oku['adres']." ".$uyeleri_oku['ilce']." ".$uyeleri_oku['sehir'] ?>   İletişim : <?= $uyeleri_oku['telefon'] ?>				
								"
							/>
							<?php $sira++;
						} ?>
				...
				<div class="pagination special-form" style="display: none;">
					<a href="?modul=<?= re("modul") ?>&sayfa=<?= re("sayfa") ?>&page=1"> <i class="fad fa-chevron-double-left"></i> </a>
					<a href="<?= (!is_numeric(re("page")) || re("page") <= 1) ? 'javascript:void(0)' : '?modul='.re("modul").'&sayfa='.re("sayfa").'&page='.(re("page") - 1).'' ?>" <?= (re("page") <= 1 || !is_numeric(re("page")) ) ? "class='page-disable'" : "" ?>> <i class="fad fa-chevron-left"></i> </a>

					<?php 
					$pagination_limit = 5;
					if($page > $pagination_limit){
					$bas_baslangic = (intval($page / $pagination_limit)) * $pagination_limit;
					$bas_son = ((intval($page / $pagination_limit)) * ($pagination_limit * 2) +1);
					} else {
					$bas_baslangic = 1;
					$bas_son = $pagination_limit+1;
					}
					for ($i=$bas_baslangic; $i <= $total_page; $i++) { 
					$hesap_sayfa = $i;
					if($hesap_sayfa <= $total_page) {
						?>
						<a href="?modul=<?= re("modul") ?>&sayfa=<?= re("sayfa") ?>&page=<?= $hesap_sayfa ?>" <?= ($page == $hesap_sayfa) ? "class='page-active'" : ""  ?> > <?= $hesap_sayfa ?> </a>
						<?php
					}
					}
					?>

					<a href="<?= (re("page") < $total_page) ? '?modul='.re("modul").'&sayfa='.re("sayfa").'&page='.(re("page") + 1).'' : 'javascript:void(0)' ?>" <?= (re("page") >= $total_page) ? "class='page-disable'" : "" ?> > <i class="fad fa-chevron-right"></i> </a>
					<a href="?modul=<?= re("modul") ?>&sayfa=<?= re("sayfa") ?>&page=<?= $total_page ?>"> <i class="fad fa-chevron-double-right"></i> </a>
				</div>
			</div>
		</form>
	<?php } ?>
  ...
  cursor: not-allowed;
   ...
   ?>
	...
	<script>
			...
			jQuery.ajax({
				...
				success: function(response) {
					if(response.status==200){
						...
						var text='<a id=\''+uye_id+'\' class=\'view_notlari\'><i class=\'fas fa-align-left\'>'+response.toplam_not+'</i></a>';
						$('#td_view_notlari_'+uye_id).html(text);
					}else{
		...
		function tanitim_sms_gonder(uye_id){
			jQuery.ajax({
				url: "https://ihale.pertdunyasi.com/check.php",
				type: "POST",
				dataType: "JSON",
				data: {
					action: "tanitim_sms_gonder",
					id: uye_id,
				},
				success: function(response) {
					if(response.status==200){
						openToastrSuccess("Tanıtım sms'i gönderildi");
		...
		function sifre_yenile_sms(uye_id){
			jQuery.ajax({
				url: "https://ihale.pertdunyasi.com/check.php",
				type: "POST",
				dataType: "JSON",
				data: {
					action: "sifre_hatirlat_sms",
					id: uye_id,
				},
				success: function(response) {
					if(response.status==200){
						openToastrSuccess("Şifre hatrılatma sms'i gönderildi");
						alert(response.message)
					}else{
						alert(response.message)
					}
		...
		function sifre_yenile(uye_id){
			openToastrSuccess("Şifre hatrılatma email'i gönderildi");
			jQuery.ajax({
