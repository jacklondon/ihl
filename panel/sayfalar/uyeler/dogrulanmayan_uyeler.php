<?php 
	...
	$uyeleri_cek = mysql_query("SELECT user.*,onayli_kullanicilar.id as o_id FROM `user` inner join onayli_kullanicilar on user.id=onayli_kullanicilar.user_id where onayli_kullanicilar.durum=0 order by id desc");
...
?>
				...
				<table width="100%" class="table table-bordered">
					...
					<tbody>
						...
						<?php $count=1; while($filtre_oku = mysql_fetch_array($result)){ 
							...
							$kazandigini_cek = mysql_query("SELECT * FROM kazanilanlar WHERE uye_id = '".$gelen_id."'");
						...
						<tr>
							...
							<td class="dropdown">
								...
								<ul class="dropdown-menu"style="position: absolute; transform: translate3d(90px, 10px, 0px); top: -30px; left: -240px; will-change: transform;">
									...
									<li><a href="javascript:void(0)" onclick="dogrulama_yenile_sms(<?= $filtre_oku['id'] ?>);">Doğrulama Hatırlat (Sms)</a></li>
									<li><a href="javascript:void(0)" onclick="dogrulama_yenile(<?=$filtre_oku['id'] ?>); alert('Mail gönderildi'); ">Doğrulama Hatırlat (Eposta)</a></li>
							...
							<tr>
								...
								<td class="dropdown">
									...
									<ul class="dropdown-menu"style="position: absolute; transform: translate3d(90px, 10px, 0px); top: -30px; left: -240px; will-change: transform;">
										...
										<li><a href="javascript:void(0)" onclick="dogrulama_yenile_sms(<?= $uyeleri_oku['id'] ?>);">Doğrulama Hatırlat (Sms)</a></li>
										<li><a href="javascript:void(0)" onclick="dogrulama_yenile(<?=$uyeleri_oku['id'] ?>); alert('Mail gönderildi'); ">Doğrulama Hatırlat (Eposta)</a></li>
   ...
   ?>
	...
	<script>
		...
		function dogrulama_yenile_sms(uye_id){
			jQuery.ajax({
				url: "https://ihale.pertdunyasi.com/check.php",
				type: "POST",
				dataType: "JSON",
				data: {
					action: "dogrulama_hatirlat_sms",
					id: uye_id,
				},
				success: function(response) {
					if(response.status==200){
		...
		function dogrulama_yenile(uye_id){
			jQuery.ajax({
				url: "?modul=ayarlar&sayfa=sifre_mail",
				type: "POST",
				dataType: "JSON",
				data: {
					action: "dogrulama_yenile",
					id: uye_id,
				},
				success: function(response) {
