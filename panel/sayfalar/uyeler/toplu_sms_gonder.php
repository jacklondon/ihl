...
<?php
	...
	{                    
		...
		if($f_telefon !=""){
			$where .= "AND concat(telefon,sabit_tel) LIKE '%".$f_telefon."%'";
		}
		if($f_mail !=""){
		...
		{
		   $sayac=count($_POST["gonderilecek_kisiler"]);
			if($sayac<1000)
			{
				for($i=0;$i<$sayac;$i++)
				{
					  coklu_sms_gonder($_POST["gonderilecek_kisiler"][$i],re("icerik"),2);
				}
			}else{
				echo "<script>alert('SMS sayısı 1000 adeti geçmektedir. ')</script>";
			}
		}
	...
	?> 
			...
			<form name="form" id="form" method="POST" enctype="multipart/form-data">
				...
				<?php 
					/*
					<select multiple="multiple" name="gonderilecek_kisiler[]" class="span12" required style="min-height:350px">
					<?php while($uye_oku = mysql_fetch_array($result)){ ?>
					<option value="<?= $uye_oku['id'] ?>"><?= $uye_oku['ad'] ?></option>
					<?php } ?>
					</select>
					*/
				?>
				<?php while($uye_oku = mysql_fetch_array($result)){ ?>
					<input type="hidden" name="gonderilecek_kisiler[]" value="<?= $uye_oku['id'] ?>">
