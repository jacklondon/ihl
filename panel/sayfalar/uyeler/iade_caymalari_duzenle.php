<?php 
	...
	if(re("action")=="iade_guncelle"){
		
		$iade_id = $_POST['iade_id'];
		...
		$tutar_cek=mysql_query("Select * from cayma_bedelleri where id='".$iade_id."'");
		$aktif_tutari_cek=mysql_fetch_assoc($tutar_cek);
		//$tutar=$aktif_tutari_cek["tutar"];
		
		
		$gelen_tutar_cek=mysql_query("select * from cayma_bedelleri where id='".$aktif_tutari_cek["cayma_id"]."'");
		$gelen_tutar_oku=mysql_fetch_array($gelen_tutar_cek);
		$tutar=$gelen_tutar_oku["tutar"];
		...
		if($depozito_durumu == "İptal Edildi"){
			...
			if($net > 0){
				...
				mysql_query("
								INSERT
									INTO
								`cayma_bedelleri` (`id`, `uye_id`, `tutar`, `hesap_sahibi`, `iban`, `uye_grubu`, `tarih`, `iade_tarihi`, `iade_tutari`, `aciklama`, `net`, `durum`, `sonuc`,`cayma_id`) 
									VALUES 
								(NULL, '".$gelen_id."', '".$net."', '".$hesap_sahibi."', '".$aktif_iban."', '".$uyenin_grubu."', '".$date."', '".$date."', '".$aktif_iade_tutar."', '".$aktif_aciklama."', '".$net."', '4', '".$depozito_durumu."','".$iade_id."')
							");
			}else{
				mysql_query("
								UPDATE 
