<?php 
	...
	if ( re('islem') == "tasi" )
	{
		...
		{
			$tut=false;
			$listeyi_sor_is=mysql_query("select * from urunler_kategoriler where kategori='".re('si')."' order by sira");
			...
			mysql_query("update urunler_kategoriler set sira='".$basilan_sira."' where id='".$sonraki_id."'");
			mysql_query("update urunler_kategoriler set sira='".$sonraki_sira."' where id='".$basilan_id."'");
		}
		
		if ( re('gorev') == "up" )
		{
			$birak=true;
			$listeyi_sor_is=mysql_query("select * from urunler_kategoriler where kategori='".re('mgrup')."' order by sira");
			while( $listeyi_cek_is=mysql_fetch_array($listeyi_sor_is) )
			{
			...
			mysql_query("update urunler_kategoriler set sira='".$basilan_sira."' where id='".$onceki_id."'");
			mysql_query("update urunler_kategoriler set sira='".$onceki_sira."' where id='".$basilan_id."'");
	...
	if ( re('islem') == "KAYDET" )
	{
		...
		$listeyi_sor_isle=mysql_query("select * from urunler_kategoriler where kategori='".re('si')."' order by sira");
		while( $listeyi_cek_isle=mysql_fetch_array($listeyi_sor_isle) )
		{
			...
			else
			{
				...
				if ($listeyi_cek_isle['bilgi'] != re('bilgi_'.$listeyi_cek_isle['id']) ) { $guncelle=true; }
				
				if($guncelle == true)
				{
					mysql_query("update urunler_kategoriler set tanim='".re('kategori_adi_'.$listeyi_cek_isle['id'])."', bilgi='".re('bilgi_'.$listeyi_cek_isle['id'])."' where id='".$listeyi_cek_isle['id']."'");
				}
			}
	...
	$listeyi_sor_on=mysql_query("select * from urunler_kategoriler where kategori='".re('si')."' order by sira");
	...
	$listeyi_sor=mysql_query("select * from urunler_kategoriler where kategori='".re('si')."' order by sira");
	while( $listeyi_cek=mysql_fetch_array($listeyi_sor) )
	{
		...
		$liste.='<tr>
						...
						<td><input type="text" name="bilgi_'.$listeyi_cek['id'].'" value="'.$listeyi_cek['bilgi'].'" style="width:90%"></td>
						<td>
							'.$acik_kapali.'
						</td>
						<td>
							<a href="?modul=urunler&sayfa=kategoriler&si='.$listeyi_cek['id'].'" class="btn mini green"><i class="icon-edit"></i>Alt Kategori</a>
						</td>
						<td>	
						...
						<td>
							<a href="#" class="btn mini green"><i class="icon-edit" title="Ek Bilgi"></i>EB</a>
						</td>
						<td>
						...
						<td>
							<a href="?modul=urunler&sayfa=kategoriler_logo&kategori='.$listeyi_cek['id'].'" class="btn mini green"><i class="icon-edit"></i>Logo</a>
						</td>
