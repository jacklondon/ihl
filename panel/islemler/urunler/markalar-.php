<?php 
	...
	if ( re('islem') == "KAYDET" )
	{
		...
		$listeyi_sor_isle=mysql_query("select * from urunler_markalar order by sira");
		while( $listeyi_cek_isle=mysql_fetch_array($listeyi_sor_isle) )
		{
			...
			else
			{
				...
				if($guncelle == true)
				{
					mysql_query("update urunler_markalar set tanim='".re('kategori_adi_'.$listeyi_cek_isle['id'])."' where id='".$listeyi_cek_isle['id']."'");
				}
			}
	...
	$sayi=0;
	$liste='';
	$listeyi_sor=mysql_query("select * from urunler_markalar order by sira");
