<?php 
	...
	if ( re('islem') == "tasi" )
	{
		...
		{
			$tut=false;
			$listeyi_sor_is=mysql_query("select * from urunler_kataloglar where kategori='".re('si')."' order by sira");
			...
			mysql_query("update urunler_kataloglar set sira='".$basilan_sira."' where id='".$sonraki_id."'");
			mysql_query("update urunler_kataloglar set sira='".$sonraki_sira."' where id='".$basilan_id."'");
		}
		
		if ( re('gorev') == "up" )
		{
			$birak=true;
			$listeyi_sor_is=mysql_query("select * from urunler_kataloglar where kategori='".re('mgrup')."' order by sira");
			while( $listeyi_cek_is=mysql_fetch_array($listeyi_sor_is) )
			{
			...
			mysql_query("update urunler_kataloglar set sira='".$basilan_sira."' where id='".$onceki_id."'");
			mysql_query("update urunler_kataloglar set sira='".$onceki_sira."' where id='".$basilan_id."'");
	...
	if ( re('islem') == "ac_kapa" )
	{
		...
		mysql_query("update urunler_kataloglar set durum='".$durum."' where id='".re('mgrup')."'");
	}
		
	if ( re('islem') == "KAYDET" )
	{
		/* Önce silinecekleri silsin, sonra güncelllenecekleri güncellesin sonra ise ekleme yapsın. */
		$listeyi_sor_isle=mysql_query("select * from urunler_kataloglar where kategori='".re('si')."' order by sira");
		while( $listeyi_cek_isle=mysql_fetch_array($listeyi_sor_isle) )
		{
			if ( re('sec_'.$listeyi_cek_isle['id']) == $listeyi_cek_isle['id'] )
			{
				mysql_query("delete from urunler_kataloglar where id='".$listeyi_cek_isle['id']."'")or die(mysql_error());
			}
			else
			{
				$guncelle=false;
				...
				if($guncelle == true)
				{
					mysql_query("update urunler_kataloglar set tanim='".re('kategori_adi_'.$listeyi_cek_isle['id'])."' where id='".$listeyi_cek_isle['id']."'");
				}
			}
		...
		{
			$simdiki_sira++;
			mysql_query("insert into urunler_kataloglar (
			id,
			kategori,
			sira,
			tanim,
	...
	$listeyi_sor_on=mysql_query("select * from urunler_kataloglar where kategori='".re('si')."' order by sira");
	...
	$sayi=0;
	$liste='';
	$listeyi_sor=mysql_query("select * from urunler_kataloglar where kategori='".re('si')."' order by sira");
	while( $listeyi_cek=mysql_fetch_array($listeyi_sor) )
	{
		...
		$liste.='<tr>
						...
						<td>
							<a href="#" class="btn mini green"><i class="icon-edit"></i>Logo</a>
						</td>
